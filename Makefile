...
PTX_FILE := vectorAdd_kernel${TARGET_SIZE}.ptx
CUBIN_FILE := vectorAdd_kernel.cubin
...
#build: vectorAddDrv
build: toy_host  toy_kernel.cubin
#build: vectorAddDrv $(CUBIN_FILE)
...
toy_kernel.cubin: toy_kernel.cu
	$(EXEC) $(NVCC) $(INCLUDES) $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -cubin $<

toy_host.o:toy_host.cpp
...
clean:
	rm -f toy_host toy_host.o 
	rm -rf ../../bin/$(TARGET_ARCH)/$(TARGET_OS)/$(BUILD_TYPE)/vectorAddDrv
	rm -rf ../../bin/$(TARGET_ARCH)/$(TARGET_OS)/$(BUILD_TYPE)/$(CUBIN_FILE)
